...
function spline:append_arc(x, y, z, rx, ry, rz, ux, uy, uz, radian_start, radian_end, divisions)
...
function spline:append_rect(cx, cy, cz, rx, ry, rz, ux, uy, uz)
	self:add_point(cx + rx + ux, cy + ry + uy, cz + rz + uz)
	self:add_point(cx + rx - ux, cy + ry - uy, cz + rz - uz)
	self:add_point(cx - rx - ux, cy - ry - uy, cz - rz - uz)
	self:add_point(cx - rx + ux, cy - ry + uy, cz - rz + uz)
	self:add_point(cx + rx + ux, cy + ry + uy, cz + rz + uz)
...
local function calc_point_at_distance(x, y, z, tx, ty, tz)
	if px then
		...
		if len >= remaining_length then
			...
			local t = rest / len
			px, py, pz = lerp3d(1 - t, px, py, pz, x, y, z)
			ptx, pty, ptz = normalize3d(ptx, pty, ptz)
			tx, ty, tz = normalize3d(tx, ty, tz)
			ptx, pty, ptz = normalize3d(lerp3d(1 - t, ptx, pty, ptz, tx, ty, tz))
			return true
		end
...
function spline:calc_point(segment, t, debug)
	...
	local t2x, t2y, t2z = unpack(p2.t_in)
	if t1x == 0 and t1y == 0 and t1z == 0 and t2x == 0 and t2y == 0 and t2z == 0 then
		local x, y, z = lerp3d(t, x1, y1, z1, x2, y2, z2)
	...
	local ax, ay, az = lerp3d(t, x1, y1, z1, x1t, y1t, z1t)
	local bx, by, bz = lerp3d(t, x1t, y1t, z1t, x2t, y2t, z2t)
	local cx, cy, cz = lerp3d(t, x2t, y2t, z2t, x2, y2, z2)
